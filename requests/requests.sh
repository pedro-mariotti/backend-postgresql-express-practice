...
curl --request POST \--url http://localhost:5000/users/register \--header 'Content-Type: application/json' \--data '{"username":"usertesteLocal","password":"senhateste01","email":"usertesteLocal@example.com"}'

# (ERRO) Testar registrar um usuario com email ja existente
curl --request POST \--url http://localhost:5000/users/register \--header 'Content-Type: application/json' \--data '{"username":"usertestemail03","password":"senhateste01","email":"usertesterepetido@example.com"}'
...
curl --request POST \--url http://localhost:5000/users/login \--header 'Content-Type: application/json' \--data '{"username":"userTesteLogin","password":"senhatesteLogin"}'
...
curl --request GET \--url http://localhost:5000/protected \--header 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY4MTI2ZTFlNWVlZjc3YzFhZTk3MDNhNyIsImlhdCI6MTc0NjA0MDk3NiwiZXhwIjoxNzQ2MDQ0NTc2fQ.aDXWrZsyvGZntHgS7TRM1f68StV1eqPm6Z6bsxTc8yc'
